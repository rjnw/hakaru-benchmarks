...
(define (run-test classes points)
  ...
  (define outfile (build-path output-dir testname "rkt-sham/no-opt" (format "~a-~a" classes points)))
  ...
  (define (run-single str out-port)
    ...
    (gibbs-timer spr zsc printer #:min-time 0 #:step-time 0.01 #:min-sweeps 10 #:step-sweeps 1)

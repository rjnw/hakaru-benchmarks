...
(define (run-test n)
  ...
  (define module-env (compile-file srcfile (list (list)
                                                 (list))))
  ...
  (define make-pair-array-bool
    (jit-get-function (string->symbol (format "make$pair<array<bool>*.array<bool>*>"))
                      module-env))
  (define make-array-bool
    (jit-get-function (string->symbol (format "new-sized$array<bool>"))
                      module-env))
  (define set-index-array-bool
    (jit-get-function (string->symbol (format "set-index!$array<bool>"))
                      module-env))

  (define tbool (jit-get-racket-type 'bool module-env))

  (define (make-array lst)
    (define arr (make-array-bool (length lst)))
    (for ([v lst]
